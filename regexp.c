...
** same name.
*/
#define ReCompiled sqlite3re_compiled
#define re_match   sqlite3re_match
#define re_compile sqlite3re_compile
#define re_dup     sqlite3re_dup
...
ReCompiled *re_dup(ReCompiled *pRe){
  ReCompiled *rRe;

  if (!(rRe = sqlite3_malloc(sizeof(*rRe))))
    return rRe;
  memcpy(rRe, pRe, sizeof(*rRe));
  if (!(rRe->aOp = sqlite3_malloc64(rRe->nAlloc * sizeof(*rRe->aOp)))
   || !(rRe->aArg = sqlite3_malloc64(rRe->nAlloc * sizeof(*rRe->aArg)))) {
    sqlite3_free(rRe);
    rRe = 0;
  } else {
    memcpy(rRe->aOp, pRe->aOp, rRe->nAlloc * sizeof(*rRe->aOp));
    memcpy(rRe->aArg, pRe->aArg, rRe->nAlloc * sizeof(*rRe->aArg));
  }
  return rRe;
}
